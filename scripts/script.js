...
//телефон
const modalPhoneNumber = document.querySelector(".modal__numberItem");
const modalPhoneModal = document.querySelector(".number_submit");
const modalPhone = document.querySelector(".modal__phone__number");
//для фильтра
...
const closeModal = (event) => {
  ...
  if (
    target.closest(".modal__close") ||
    target.closest(".modal__close_phone") ||
...
card_click.addEventListener("click", (event) => {
  ...
  if (card) {
    ...
    modalPhoneNumber.textContent = item.numberItem;
    //телефон
    //modalPhoneNumber.textContent = item.document.scroll;
    modalItem.classList.remove("hide");

    modalPhoneModal.addEventListener("click", (event) => {
      console.log(target);
      closeModal({ target: modalItem });
      console.log(item);
      swal({
        title: "Good job!",
        text: `Номер: ${item.numberItem} `,
        icon: "success",
        button: "Good luck!",
      });
      document.addEventListener("keydown", closeModal);
...
modalPhone.addEventListener("click", closeModal);
